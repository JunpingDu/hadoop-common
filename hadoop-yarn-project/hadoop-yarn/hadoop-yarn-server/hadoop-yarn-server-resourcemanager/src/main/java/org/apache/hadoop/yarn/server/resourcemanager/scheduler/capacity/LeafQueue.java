...
public class LeafQueue implements CSQueue {
      ...
      synchronized (application) {
        ...
        for (Priority priority : application.getPriorities()) {
          ...
          if (Resources.greaterThan(
              resourceCalculator, assigned, Resources.none())) {
  ...
  private synchronized boolean assignToQueue(Resource clusterResource, 
    ...
    if (potentialNewCapacity > absoluteMaxCapacity) {
      LOG.info(getQueueName() + 
          ...
          " currentCapacity " + 
            Resources.divide(resourceCalculator, usedResources, 
                clusterResource) + 
          " required " + required +
  ...
  private Resource computeUserLimitAndSetHeadroom(
    ...
    Resource headroom = 
        Resources.subtract(
            Resources.min(resourceCalculator, userLimit, queueMaxCap), 
            userConsumed);
  ...
  private Resource computeUserLimit(FiCaSchedulerApp application, 
    ...
    Resource currentCapacity =
        Resources.lessThan(resourceCalculator, usedResources, queueCapacity) ?
  ...
  private CSAssignment assignContainersOnNode(Resource clusterResource, 
    ...
    assigned = 
        assignNodeLocalContainers(clusterResource, node, application, priority,
            reservedContainer); 
    if (Resources.greaterThan(resourceCalculator, assigned, Resources.none())) {
  ...
  private Resource assignContainer(Resource clusterResource, FiCaSchedulerNode node, 
    ...
    assert Resources.greaterThan(resourceCalculator, available, 
        Resources.none());
